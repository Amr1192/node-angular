...
exports.getAllExams = catchAsync(async (req, res, next) => {
  // Get all exams
  const exams = await Exam.find().select("title description totalMarks"); // Updated to match interface
...
exports.submitExam = catchAsync(async (req, res, next) => {
  ...
  exam.questions.forEach((question, index) => {
    ...
    if (userAnswer) {
      const selectedOption = question.options[userAnswer.selectedChoiceIndex]; // Updated to use options
